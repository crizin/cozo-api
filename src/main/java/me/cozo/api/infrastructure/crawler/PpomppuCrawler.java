...
public class PpomppuCrawler extends Crawler {
	...
	protected Set<Article> getArticles(Board board, Document document) {
		return document.select("#revolution_main_table tbody tr[class^=common-list]").stream()
			.filter(tr -> !tr.html().contains("해당글은 운영자에 의해 블라인드 처리된 글입니다."))
			.map(tr -> Article.builder()
				.board(board)
				.originId(TextUtils.getFragment(attr(tr, "td:eq(2) a", "href"), "no=(\\d+)"))
				.title(textOnly(tr, "td:eq(2) a font.list_title"))
				.hits(number(tr, "td:eq(5)"))
				.comments(number(tr, "td:eq(2) span.list_comment2 span"))
